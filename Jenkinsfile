    ...
    // -------------------------------------------------------------------------
 	...
 	withEnv(["HOME=${env.WORKSPACE}"]) {	
		...
		stage('Authorize to Salesforce') {
			...
			rc = command '$toolbelt/sfdx auth:jwt:grant --instanceurl $SF_INSTANCE_URL --clientid $SF_CONSUMER_KEY --username $SF_USERNAME --jwtkeyfile $server_key_file --setdefaultdevhubusername --setalias UAT'
		    if (rc != 0) {
