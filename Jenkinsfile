    ...
    // -------------------------------------------------------------------------
 	...
 	withEnv(["HOME=${env.WORKSPACE}"]) {	
		...
		stage('Authorize to Salesforce') {
			...
			rc = command "${toolbelt}/sfdx auth:jwt:grant --instanceurl ${SF_INSTANCE_URL} --clientid ${SF_CONSUMER_KEY} --jwtkeyfile /tmp/server.key --username ${SF_USERNAME} --setalias UAT"
		    if (rc != 0) {
